...
bert_tokenizer_en = BertTokenizer.from_pretrained("/home/premyu/bert/en/")
bert_tokenizer_de = BertTokenizer.from_pretrained("/home/premyu/bert/de/")
...
class Encoder(nn.Module):
    def __init__(self, 
        ...
        self.pos_embedding = PostionalEncoding(max_length, hid_dim)
...
model_log = os.path.join(output_path, 'model.log')
lr_log = os.path.join(output_path, 'lr.log')

with open(model_log, 'w') as f:
        f.write('epoch,train_loss,train_ppl,valid_loss,valid_ppl,time_sec\n')

with open(lr_log, 'w') as g:
        g.write('learning_rate\n')


##################################################################################################################

start_dt = datetime.now()
print("Training start:", start_dt)
...
for epoch in range(N_EPOCHS):
    ...
    if valid_loss < best_valid_loss:
        ...
        save_path = os.path.join(output_path, 'model_checkpoint-{}.pt'.format(epoch))
        #save_path = os.path.join(output_path, 'model_checkpoint.pt'.format(epoch))
        torch.save(model.state_dict(), save_path)

        

    #save_path = os.path.join(checkpoint_path, 'state-{}.pt'.format(epoch))
    ...
    print(f'Epoch: {epoch+1:03},Time: {epoch_mins}m {epoch_secs}s | Train Loss: {train_loss:.3f} | Train PPL: {train_ppl:7.3f} | Val. Loss: {valid_loss:.3f} | Val. PPL: {valid_ppl:7.3f}')
    print(f'Learning rate:{lr}')

    with open(model_log, 'a') as f:
        f.write('{epoch},{train_loss:.3f},{train_ppl:7.3f},{valid_loss:.3f},{valid_ppl:7.3f},{time_sec}\n'.format(
                    epoch=epoch+1, train_loss=train_loss, train_ppl=train_ppl, 
                    valid_loss=valid_loss, valid_ppl=valid_ppl, time_sec=time_sec))
        
    with open(lr_log, 'a') as g:
      g.write(str(lr)+'\n')

end_dt = datetime.now() # time object
print("Training end:", end_dt)
